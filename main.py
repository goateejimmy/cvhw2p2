...
def train_interface():
    ...
    milestones = cfg['milestones']#scheduler
    ...
    train_set, val_set =  get_cifar10_train_val_set(root=data_root, ratio=split_ratio)    # 3 32 32 , 1
    train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True) # 8 3 32 32 , 8 1
    val_loader = DataLoader(val_set, batch_size=batch_size, shuffle=False)
    
    # define your loss function and optimizer to unpdate the model's parameters.
    
    optimizer = optim.SGD(model.parameters(), lr=lr, momentum=0.9,weight_decay=1e-6, nesterov=True)#全部的model變數都做更改
